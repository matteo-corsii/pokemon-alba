...
Assert-True (@($map.object_events).Count -eq 15) 'Lago must contain exactly twelve ambient NPCs, Lauro, Lia and Nico.'
...
Assert-True (@($map.warp_events).Count -eq 12 -and @($map.coord_events).Count -eq 1) 'Lago warp or coordinate event counts are incorrect.'
