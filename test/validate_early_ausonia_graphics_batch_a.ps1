...
{
    # Index 0 is transparent in the PNG. Its RGB value is deliberately magenta,
    # while the engine loads the selected global icon palette at runtime.
...
$entries = @(
    [pscustomobject]@{ Name='BORGOTTO'; Symbol='Borgotto'; Folder='borgotto'; Placeholder='Lillipup'; IconPalette=2; FrontBox=@(7,5,48,55); BackBox=@(10,14,42,46); FrontSize='MON_COORDS_SIZE(48, 56)'; BackSize='MON_COORDS_SIZE(48, 48)'; Offset=4 },
    [pscustomobject]@{ Name='PASTUFO'; Symbol='Pastufo'; Folder='pastufo'; Placeholder='Herdier'; IconPalette=1; FrontBox=@(5,3,54,58); BackBox=@(6,3,52,58); FrontSize='MON_COORDS_SIZE(56, 64)'; BackSize='MON_COORDS_SIZE(56, 64)'; Offset=3 },
    [pscustomobject]@{ Name='MICIOLO'; Symbol='Miciolo'; Folder='miciolo'; Placeholder='Skitty'; IconPalette=0; FrontBox=@(9,3,46,58); BackBox=@(8,3,47,58); FrontSize='MON_COORDS_SIZE(48, 64)'; BackSize='MON_COORDS_SIZE(48, 64)'; Offset=3 },
    [pscustomobject]@{ Name='FELIVATES'; Symbol='Felivates'; Folder='felivates'; Placeholder='Espeon'; IconPalette=1; FrontBox=@(6,3,52,58); BackBox=@(6,3,52,58); FrontSize='MON_COORDS_SIZE(56, 64)'; BackSize='MON_COORDS_SIZE(56, 64)'; Offset=3 }
)
...
foreach ($entry in $entries)
{
    ...
    foreach ($reference in @(
        ...
        ".frontPicYOffset = $($entry.Offset)", '.frontAnimFrames = sAnims_SingleFramePlaceHolder',
        ".backPic = gMonBackPic_$($entry.Symbol)", ".backPicSize = $($entry.BackSize)",
        ".backPicYOffset = $($entry.Offset)", ".palette = gMonPalette_$($entry.Symbol)",
...
$allowedPaths += @('test/validate_lumella_omphalux.ps1','test/validate_paludix_sanguilex.ps1','graphics/pokemon/lumella/anim_front.png','graphics/pokemon/lumella/back.png','graphics/pokemon/lumella/icon.png','graphics/pokemon/lumella/normal.pal','graphics/pokemon/lumella/shiny.pal','graphics/pokemon/omphalux/anim_front.png','graphics/pokemon/omphalux/back.png','graphics/pokemon/omphalux/icon.png','graphics/pokemon/omphalux/normal.pal','graphics/pokemon/omphalux/shiny.pal','graphics/pokemon/paludix/anim_front.png','graphics/pokemon/paludix/back.png','graphics/pokemon/paludix/icon.png','graphics/pokemon/paludix/normal.pal','graphics/pokemon/paludix/shiny.pal','graphics/pokemon/sanguilex/anim_front.png','graphics/pokemon/sanguilex/back.png','graphics/pokemon/sanguilex/icon.png','graphics/pokemon/sanguilex/normal.pal','graphics/pokemon/sanguilex/shiny.pal')
foreach ($entry in $entries)
{
...
Write-Output 'Early Ausonia graphics batch A validation passed.'
