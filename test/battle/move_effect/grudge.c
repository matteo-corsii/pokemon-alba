...
SINGLE_BATTLE_TEST("Grudge depletes all PP from a Max Move's base move")
{
    GIVEN {
        ...
        MESSAGE("The opposing Wobbuffet lost all of GRAFFIO's PP due to the grudge!");
        MESSAGE("Wobbuffet non ha\npiù energie!");
    } THEN {
        EXPECT_GT(opponent->pp[0], 0);
        EXPECT_EQ(opponent->pp[1], 0);
...
SINGLE_BATTLE_TEST("Grudge's effect disappears if the user takes a new turn - Move")
{
    ...
    SCENE {
        ...
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SCRATCH, opponent);
        NOT MESSAGE("The opposing Wobbuffet lost all of GRAFFIO's PP due to the grudge!");
        MESSAGE("Wobbuffet non ha\npiù energie!");
    }
    THEN {
        EXPECT_GT(opponent->pp[0], 0);
