...
$newGameScripts = Read-Text 'data/scripts/new_game.inc'
$gymScripts = Read-Text 'data/maps/AlberaStorica_Anfiteatro/scripts.inc'
...
$nico = @($albera.object_events | Where-Object { $_.local_id -eq 'LOCALID_ALBERA_STORICA_NICO' -and $_.flag -eq 'FLAG_HIDE_ALBERA_STORICA_NICO' })
Assert-True ($nico.Count -eq 1) 'Nico post-Gym object must use its dedicated technical visibility flag.'
Assert-True ($flags -match '#define\s+FLAG_HIDE_ALBERA_STORICA_NICO\s+0x8ED\b') 'Missing Emerald Nico technical visibility flag.'
Assert-True ($flagsFrlg -match '#define\s+FLAG_HIDE_ALBERA_STORICA_NICO\s+0x8ED\b') 'Missing FRLG Nico technical visibility flag.'
Assert-True ((@($albera.object_events | Where-Object { $_.local_id -eq 'LOCALID_ALBERA_STORICA_ORIENTATION_NPC' -and $_.x -eq 33 -and $_.y -eq 11 }).Count -eq 1) -and (@($albera.object_events | Where-Object { $_.local_id -eq 'LOCALID_ALBERA_STORICA_PLAZA_NPC' -and $_.x -eq 18 -and $_.y -eq 14 }).Count -eq 1) -and (@($albera.object_events | Where-Object { $_.local_id -eq 'LOCALID_ALBERA_STORICA_GYM_NPC' -and $_.x -eq 18 -and $_.y -eq 9 }).Count -eq 1) -and ($nico[0].x -eq 18 -and $nico[0].y -eq 6)) 'Approved Albera exterior NPC repositioning changed.'
Assert-True ($newGameScripts -match 'EventScript_ResetAllMapFlags::[\s\S]*?setflag FLAG_HIDE_ALBERA_STORICA_NICO') 'New Emerald games must start with Nico hidden before the Badge.'
Assert-True ($newGameScripts -match 'EventScript_ResetAllMapFlagsFrlg::[\s\S]*?setflag FLAG_HIDE_ALBERA_STORICA_NICO') 'New FRLG games must start with Nico hidden before the Badge.'
Assert-True ($alberaScripts -match '(?s)AlberaStorica_OnLoad:\s*goto_if_unset FLAG_BADGE01_GET, AlberaStorica_OnLoad_HideNico\s*goto_if_set FLAG_ALBERA_NICO_GYM_ENCOUNTER_COMPLETE, AlberaStorica_OnLoad_HideNico\s*clearflag FLAG_HIDE_ALBERA_STORICA_NICO\s*end\s*AlberaStorica_OnLoad_HideNico:\s*setflag FLAG_HIDE_ALBERA_STORICA_NICO\s*end') 'Nico visibility must be synchronized by the pre-spawn map-load flag path.'
Assert-True ($gymScripts -match '(?s)AlberaStorica_Anfiteatro_EventScript_LirioDefeated:\s*setflag FLAG_BADGE01_GET\s*clearflag FLAG_HIDE_ALBERA_STORICA_NICO') 'Lirio victory must enable Nico before the next Albera Storica load.'
Assert-True ($alberaScripts -match '(?s)AlberaStorica_EventScript_NicoPostGym::[\s\S]*?setflag FLAG_ALBERA_NICO_GYM_ENCOUNTER_COMPLETE\s*setflag FLAG_HIDE_ALBERA_STORICA_NICO\s*removeobject LOCALID_ALBERA_STORICA_NICO') 'Nico encounter must complete once and hide Nico persistently.'
Assert-True ($alberaScripts -notmatch 'showobjectat|hideobjectat') 'Nico visibility must not depend on already-active object events.'
