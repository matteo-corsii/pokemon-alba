...
Assert-True ($archive.layout -eq 'LAYOUT_RUSTBORO_CITY_POKEMON_SCHOOL') 'Archive must reuse the school-compatible archive layout.'
Assert-True ($shop.layout -eq 'LAYOUT_FORTREE_CITY_DECORATION_SHOP') 'Shop must reuse the decoration-shop layout.'
Assert-True ((@($albera.warp_events | Where-Object { $_.x -eq 7 -and $_.y -eq 7 -and $_.dest_map -eq 'MAP_ALBERA_STORICA_CIVIC_ARCHIVE' })).Count -eq 1) 'Archive exterior warp is invalid.'
Assert-True ((@($albera.warp_events | Where-Object { $_.x -eq 29 -and $_.y -eq 7 -and $_.dest_map -eq 'MAP_ALBERA_STORICA_SECRET_BASE_SHOP' })).Count -eq 1) 'Shop exterior warp is invalid.'
