...
Assert-True (@($map.warp_events | Where-Object { $_.x -eq 31 -and $_.y -eq 63 -and $_.elevation -eq 0 }).Count -eq 1) 'west warp elevation'
Assert-True (@($map.warp_events | Where-Object { $_.x -eq 32 -and $_.y -eq 63 -and $_.elevation -eq 0 }).Count -eq 1) 'east warp elevation'
Assert-True (@($map.coord_events | Where-Object { $_.x -eq 31 -and $_.y -eq 63 -and $_.script -eq 'BoscoDelRomitorio_EventScript_ExitWest' }).Count -eq 1) 'west exit coord event'
Assert-True (@($map.coord_events | Where-Object { $_.x -eq 32 -and $_.y -eq 63 -and $_.script -eq 'BoscoDelRomitorio_EventScript_ExitEast' }).Count -eq 1) 'east exit coord event'
$scripts = Get-Content -Raw $scriptsPath
Assert-True ($scripts -match 'BoscoDelRomitorio_EventScript_ExitWest::\r?\n\s*warp MAP_LAGO_DI_ALBERA, 4, 2\r?\n\s*waitstate') 'west exit script'
Assert-True ($scripts -match 'BoscoDelRomitorio_EventScript_ExitEast::\r?\n\s*warp MAP_LAGO_DI_ALBERA, 5, 2\r?\n\s*waitstate') 'east exit script'
