...
Assert-True (@($map.object_events).Count -eq 15 -and @($map.warp_events).Count -eq 5 -and @($map.coord_events).Count -eq 1 -and @($map.bg_events).Count -eq 8) 'Lago event counts are incorrect.'
